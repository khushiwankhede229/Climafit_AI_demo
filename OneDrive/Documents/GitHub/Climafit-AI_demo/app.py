...
def analytics():
    ...
    # Heatmap Data (Temperature vs Fabric)
    temp_fabric_heatmap = {}
    if not df.empty and 'Temperature_C' in df.columns and 'Fabric' in df.columns:
        df_plot = df.copy()
        df_plot['Temp_Bin'] = pd.cut(df_plot['Temperature_C'], bins=[0, 15, 25, 35, 50], labels=['<15C', '15-25C', '25-35C', '>35C'])
        crosstab = pd.crosstab(df_plot['Fabric'], df_plot['Temp_Bin'])
        temp_fabric_heatmap = {
    ...
    # 3D Scatter Data
    scatter_3d_data = {}
    if not df.empty and 'Temperature_C' in df.columns and 'Price_INR' in df.columns:
        sample_df = df.sample(min(300, len(df))) if len(df) > 300 else df
        season_map = {'Winter': 1, 'Spring': 2, 'Summer': 3, 'Monsoon': 4, 'Autumn': 5}
        scatter_3d_data = {
            'x': sample_df['Temperature_C'].tolist(),
            'y': sample_df['Price_INR'].tolist(),
            'z': sample_df['Season'].map(season_map).fillna(3).tolist(),
            'text': (sample_df['Product Name'] + ' (' + sample_df['Season'] + ')').tolist(),
            'color': sample_df['Season'].map(season_map).fillna(3).tolist()
        }
    ...
    return render_template("analytics.html", 
                           ...
                           temp_fabric_heatmap=json.dumps(temp_fabric_heatmap),
                           scatter_3d_data=json.dumps(scatter_3d_data),
